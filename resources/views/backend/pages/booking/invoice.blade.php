...
<head>
    ...
    <title>{{ setting('app_name') }} || Booking Invoice</title>
...
<body>
    <div class="invoice-container">
        ...
        <div class="header" style="text-align:center;">
            <img src="{{ public_path(setting('primary_logo')) }}" alt="{{ setting('app_name') }}" style="width:200px"
        ...
        <div class="section" style="padding: 10px 0px 0px 0px">
            <address>
                <strong>Invoice:</strong><a href="" style="text-decoration: none">
                    {{ $hotel->invoice_no }}</a><br>
                <strong>Name:</strong> {{ $hotel->sales->customer->name }}<br>
                <strong>Visiting Country:</strong> {{ $hotel->visit_country }}<br>
                <strong>Phone:</strong> {{ $hotel->sales->customer->phone ?? null }}<br>
                <strong>Sale by:</strong> {{ $hotel->user->name ?? null }}<br>
                <strong>Sale:</strong> {{ $hotel->sale_date }}<br>
            </address>
            <address>
                {!! setting('pdf_address') !!}
            </address>
            ...
            <div style="margin-right:40px">
                <div>
                    <table class="table table-bordered" style="width:100%">
                        ...
                        <tbody>
                            <tr>
                                <td>{{ $hotel->hotel_name }} ,{{ $hotel->hotel_location }}
                                    ,{{ $hotel->visit_country }}</td>
                                <td>{{ $hotel->hotel_number_of_day }}</td>
                                <td>{{ currencyBD($hotel->sales->sale_price) }}/=</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
